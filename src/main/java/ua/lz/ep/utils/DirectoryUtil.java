...
public class DirectoryUtil {
    ...
    public static void createDirectories(Path path) {
        try {
            ...
            log.error("Permission denied: Cannot create directory at {}. Error: {}", path.toAbsolutePath(), e.getMessage());
            ...
            log.error("Access denied: You do not have permission to create the directory: " + e.getMessage());
            // Throwing a runtime exception stops application startup if the directory is mandatory
            throw new IllegalStateException("Failed to create mandatory startup directory: " + path, e);
        }
        catch (IOException e) {
            log.error("I/O error while creating directory at {}. Error: {}", path.toAbsolutePath(), e.getMessage());
